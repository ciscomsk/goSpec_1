...
func main() {
	...
	// Сравнивать массивы - ок. Массив можно использовать как ключ мапы.
	...
	// Сравнивать слайсы - нельзя. Т.к. тип ссылочный можно сравнить только с nil.
	// err - Invalid operation: []int{1, 2, 3} == []int{1, 2, 3} (the operator == is not defined on []int)
	//if []int{1, 2, 3} == []int{1, 2, 3} {
	//}

	// Сравнивать мапы - нельзя. Т.к. тип ссылочный можно сравнить только с nil.
	aMap := map[string]int{
		"a": 1,
	}
	bMap := map[string]int{
